Shader "Unlit/BlendPlayerShader"
{
	Properties
	{
		...
		_Tolerance ("Blend Tolerance", Range(0,255)) = 60
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float testr = step(ocol.g, _Tolerance / 255) * step(ocol.b, _Tolerance / 255);
				float testg = step(ocol.r, _Tolerance / 255) * step(ocol.b, _Tolerance / 255);
				float testb = step(ocol.r, _Tolerance / 255) * step(ocol.g, _Tolerance / 255);
				float test = testr + testg + testb;

				fixed4 col = fixed4( saturate(_BodyColor.rgb * ocol.r * testr + _VisorColor.rgb * ocol.g * testg + _BackColor.rgb * ocol.b * testb + ((1 - saturate(test)) * ocol.rgb)), ocol.a);
				if (mx < 0.001 ||
					abs(1 - mn / mx) < .45)
				{
					col.rgb = ocol.rgb;
